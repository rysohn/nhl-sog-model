...
if("gameDate" %in% names(train_data)) {
  train_data <- train_data %>%
    arrange(team, gameDate) %>%
    group_by(team) %>%
    mutate(
      days_since_last_game = as.numeric(gameDate - lag(gameDate)) - 1,
      is_b2b = ifelse(!is.na(days_since_last_game) & days_since_last_game == 0, 1, 0)
    ) %>%
    ungroup()
}
train_data
